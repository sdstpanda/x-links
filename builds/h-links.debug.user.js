...
(function () {
	...
	var UI = (function () {
		...
		var activate_actions = function (node, index) {
			...
			actions_close_timeout = setTimeout(function () { actions_close_timeout = null; }._w(132), 1);
			...
			for (i = 0, ii = this.types.length; i < ii; ++i) {
				...
				if (type.queue.length > 0) {
					...
					if (use_delay && type.count > 1) {
						setTimeout(function () { type.run(); }._w(200), 1); // jshint ignore:line
					}
					else {
						type.run();
					}
	...
	var EasyList = (function () {
		...
		var update_ordering = function () {
			...
			if (settings.group_by_filters) {
				...
				base_array = function () { return []; }._w(444);
				ordering = [];
			}
